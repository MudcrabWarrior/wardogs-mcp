 ...
 * Defaults to the package folder (git checkout). Set WARDOGS_HOME to move it, which the
 * .mcpb bundle does so nothing is written inside Claude Desktop's extension folder.
 */
export const HOME = process.env.WARDOGS_HOME ? path.resolve(process.env.WARDOGS_HOME) : ROOT;
